...
public class WordFinder {
    ...
    public static Map<String, Integer> getSubstrings(String haystack,
    	...
    	int needlesIndex = needles.length -1;
    	Map<String, Integer> output = new HashMap<String, Integer>();
    	
    	for (int i =0; i <= needlesIndex; i++){
    		boolean retval = haystack.contains(needles[i]);
    		if(retval){
    			output.put(needles[i], haystack.indexOf(needles[i]) ); 
    		}
    	}
        return output;
    }
